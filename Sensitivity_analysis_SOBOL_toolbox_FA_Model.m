...
%%%%% sensitivity analysis of FA model using GSAT a Global Sensitivity Analysis
%%%%% Toolbox for Matlab%%%%
...
Nominal_parameters=[1, 1, 1, 1, 1, 1,1,1,1]


pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(1)*0.1 Nominal_parameters(1)*10]), 'k1');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(2)*0.1 Nominal_parameters(2)*10]), 'k2');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(3)*0.1 Nominal_parameters(3)*10]), 'k3');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(4)*0.1 Nominal_parameters(4)*10]), 'k4');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(5)*0.1 Nominal_parameters(5)*10]), 'k5');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(6)*0.1 Nominal_parameters(6)*10]), 'k6');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(7)*0.1 Nominal_parameters(7)*10]), 'k4');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(8)*0.1 Nominal_parameters(8)*10]), 'k5');
pro = pro_AddInput(pro, @()pdf_Sobol([Nominal_parameters(9)*0.1 Nominal_parameters(9)*10]), 'k6');
...
pro = pro_SetModel(pro, @(x)mymodel_FA(x), 'model');
...
%%
sensitivity_indexes_vector=[S1 S2 S3 S4 S5 S6 S7 ]
parameter_names={'k1', 'k2', 'k3' , 'k4', 'k5', 'k6', 'y(0)'}%, 'VAI(0)', 'Fe(0)'};
...
%axis square
set(gca,'XTick', [1:7],'XTickLabel',parameter_names((index_sorted_sotols)))
xlim([0,8]);
...
%%
parameter_names_Fast={'k1', 'k2', 'k3' , 'k4', 'k5', 'k6', 'Tr(0)', 'VAI(0)', 'Fe(0)'};
...
%set(gcf, 'Position', [100 100 300 300]); 
%axis square
%set(gca,'XTick', [1:9],'XTickLabel',parameter_names_Fast(index_sorted_eFAST))
xlim([0, 10]);
%%
